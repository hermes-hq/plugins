...
export function layoutToSeats(furniture: PlacedFurniture[]): Map<string, Seat> {
  ...
  // Build set of workstation tiles — only actual DESK furniture (not tables/coffee tables)
  const workstationTiles = new Set<string>();
  for (const item of furniture) {
    const entry = getCatalogEntry(item.type);
    if (!entry || !entry.isDesk) continue;
    // Only count items with "DESK" in the type name (excludes TABLE_FRONT, COFFEE_TABLE, SMALL_TABLE)
    if (!item.type.toUpperCase().includes('DESK')) continue;
    for (let dr = 0; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
  ...
  for (const item of furniture) {
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
        ...
        // Check if this seat is near a workstation (desk with PC) — up to 2 tiles away
        let atDesk = false;
        for (let dr = -2; dr <= 2; dr++) {
          for (let dc = -2; dc <= 2; dc++) {
            if (workstationTiles.has(`${tileCol + dc},${tileRow + dr}`)) {
              atDesk = true;
              break;
            }
          }
