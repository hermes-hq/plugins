...
import { getAnimationFrames, getCatalogEntry, getOnStateType } from '../office/furnitureCatalog';
import {
  createDefaultLayout,
  getBlockedTiles,
  ...
  layoutToSeats,
  layoutToTileMap,
} from '../office/layoutSerializer';
import { findPath, getWalkableTiles, isWalkable } from '../office/tileMap';
import type {
  Character,
  FurnitureInstance,
  OfficeLayout,
...
import { CharacterState, Direction, MATRIX_EFFECT_DURATION, TILE_SIZE } from '../office/types';
...
export class OfficeState {
  ...
  private findFreeSeat(): string | null {
    // Prefer desk seats (adjacent to computer/desk furniture) over lounge seats
    for (const [uid, seat] of this.seats) {
      if (!seat.assigned && seat.atDesk) return uid;
    }
    // Fallback to any free seat
    for (const [uid, seat] of this.seats) {
